...
class View extends FieldPluginBase {
  ...
  public function render(ResultRow $values) {
    ...
    // Allow each row to override the view and display
    $options_view = $this->options['view'];

    if (isset($values->views_field_view__view_override)) {
      $options_view = $values->views_field_view__view_override;
    }

    $options_display = $this->options['display'];

    if (isset($values->views_field_view__display_override)) {
      $options_display = $values->views_field_view__display_override;
    }

    if (empty($running[$options_view][$options_display]) || $this->config->get('evil')) {
      if (!empty($options_view)) {
        $running[$options_view][$options_display] = TRUE;
        $args = [];
        ...
        $view = Views::getView($options_view);

        // Only execute and render the view if the user has access.
        if ($view->access($options_display)) {
          $view->setDisplay($options_display);
        ...
        $running[$options_view][$options_display] = FALSE;
      }
    }
    else {
