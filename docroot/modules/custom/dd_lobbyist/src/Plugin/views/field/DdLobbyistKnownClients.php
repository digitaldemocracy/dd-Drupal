...
use Drupal\dd_lobbyist\Entity\DdLobbyist;
 ...
 *
 * @ViewsField("dd_lobbyist_known_clients")
 */
class DdLobbyistKnownClients extends PrerenderList {
  /**
   * {@inheritdoc}
   */
  ...
  public function getItems(ResultRow $values) {
    ...
    if (!empty($this->options['pid'])) {
      ...
      $clients = DdLobbyist::getKnownClients($pid, $year);
      if ($clients) {
        foreach ($clients as $client) {
          $url = Link::createFromRoute($client->assoc_name, 'entity.dd_organization.canonical', array('dd_organization' => $client->oid));
